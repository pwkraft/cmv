...
### function to simulate logit results with clustered SEs
sim <- function(models, iv, cluster=NULL, ci=c(0.025,0.975), nsim = 1000){
  ...
  for(i in 1:length(models)){
    ...
    if(is.null(cluster)){
      betas <- MASS::mvrnorm(nsim, coef(models[[i]]), sandwich::vcovCL(models[[i]], cluster = cluster))
    } else {
